// Major influence from https://github.com/yoskeoka/gognito
...
// EnableCognitoAuth ... self validation....
func EnableCognitoAuth(cfg *viper.Viper) gin.HandlerFunc {
	...
	region := cfg.GetString("COGNITO_REGION")
	userPoolID := cfg.GetString("COGNITO_USER_POOL_ID")

	if region == "" || userPoolID == "" {
		panic(fmt.Errorf("Unknown AWS Region or Userpool"))
	}

	jwkURL := fmt.Sprintf("https://cognito-idp.%v.amazonaws.com/%v/.well-known/jwks.json", region, userPoolID)
	// fmt.Println(jwkURL) // debug line
	jwk := getJWK(jwkURL)
...
func convertKey(rawE, rawN string) *rsa.PublicKey {
	...
	// fmt.Println(decodedN) // debug line
	// fmt.Println(decodedE) // debug line
	// fmt.Printf("%#v\n", *pubKey) // debug line
	return pubKey
}
